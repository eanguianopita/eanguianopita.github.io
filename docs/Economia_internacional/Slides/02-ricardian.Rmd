---
title: "I9387: Clase 2"
subtitle: "Modelo ricardiano (autarquía)"
...
# 3. Calcular los puntos de intercepción (los extremos de la FPP)
x_intercept <- L / lx
y_intercept <- L / ly

# 4. Crear los datos para la línea de la FPP y el área de producción
# Data frame para la línea que va de (0, L/ly) a (L/lx, 0)
ppf_data <- data.frame(
  x = c(0, x_intercept),
  y = c(y_intercept, 0)
)

# Data frame para el polígono que sombrea el "Production Set"
production_set_data <- data.frame(
  x = c(0, x_intercept, 0),
  y = c(0, 0, y_intercept)
)

# 5. Crear el gráfico paso a paso
ggplot() +
  # Dibuja y colorea el área del "Conjunto de Producción" (Production Set)
  geom_polygon(data = production_set_data, aes(x = x, y = y), fill = "#c1f0b2", alpha = 0.8) +
  
  # Dibuja la línea de la FPP (PPF)
  geom_line(data = ppf_data, aes(x = x, y = y), color = "blue", size = 1.5) +
  
  # Añade las etiquetas de texto
  ...
  annotate("text", x = x_intercept * 0.3, y = y_intercept * 0.3, label = "Conjunto factible de producción", 
           color = "#1e6b02", size = 4) +
...
.bigger[.center[.white[**Ventaja absoluta y comparativa (Autarquía)**]]]
...
.bigger[.center[.white[**Ejemplo en autarquía**]]]
...
# 2. Definir la función que quieres graficar
equation <- function(x) {
  50 - 0.5 * x
}
...
ggplot(data.frame(x = c(0, 100)), aes(x = x)) +
  ...
  # Dibuja la línea de la función en el rango de x (de 0 a 100)
  geom_function(fun = equation, color = "blue", linewidth = 1.5) +
  
  # Configura los límites y las marcas de los ejes para que coincidan con la imagen
  scale_x_continuous(
    limits = c(0, 100), 
  ...
  # Nombra los ejes y añade la ecuación como un pie de foto (caption)
  labs(
    x = "x", 
    y = "y",
    caption = "y = 50 – 0.5x"
  ...
  # Aplica un tema visual limpio y clásico
  theme_classic() +
  
  # Personaliza el estilo para que se parezca más a la imagen
  theme(
    axis.title = element_text(size = 14),
    axis.text = element_text(size = 12),
    plot.caption = element_text(
      hjust = 0.5,      # Centra el pie de foto
      size = 18,        # Aumenta el tamaño de la fuente
...
---
# Ventaja comparativa y precios relativos de autarquía
...
.pull-left[
.center[
Precios relativos de autarquía <br> (Costos de oportunidad)
]
...
.bigger[.center[.white[**Formalización del modelo ricardiano**]]]
...
---
# Solución de modelos competitivos 
...
.red[Importante: La producción tiene unidades naturales. No se utilizan transformaciones ⚠️]
...
.red[Importante: Esta condición cambia dependiendo de modelo en autarquía o libre comercio.]

---
# Vaciado de mercados: Autarquía 
...
---
# Variables exógenas vs endógenas
...
.bigger[.center[.white[**Solución de equilibrio: <br> Problema del consumidor**]]]
...
# Problema del consumidor: Gráficamente
...
ggplot() +
  ...
  geom_text(data = opt_point, aes(x = c1 + 0.1, y = c2 + 0.3), 
            label = "Óptimo", color = "blue", size = 5, hjust = 0) +
...
---
# Solución al problema de las empresas 
...
# Solución del equilibrio en autarquía


---

# Solución del modelo en autarquía 
...
# Solución del equilibrio (autarquía)
...
.bigger[.center[.white[**Solución del equilibrio en libre comercio**]]]


---

# Solución del equilibrio: Libre comercio
